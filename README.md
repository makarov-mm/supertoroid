...
| -------- | -------- | ---------------- | --------- | ---------------- |
| Windows  | C++      | Win32 + WGL      | 4.3 core  | `supertoroid.cpp`|
...
A single `.cpp` file; the required libraries are linked via `#pragma comment(lib, ...)`.
Needs a GPU/driver supporting an OpenGL 4.3 core context.

With the MSVC command-line tools (the linker infers the Windows subsystem from `WinMain`):

```bat
cl /EHsc /O2 supertoroid.cpp
```

Or add `supertoroid.cpp` to a new Visual Studio project and build it (x64, Release).
